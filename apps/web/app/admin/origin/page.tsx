...
export default function OriginAdmin() {
  const [list, setList] = useState<OriginChapter[]>([])
  const [selected, setSelected] = useState<OriginChapter | null>(null)
  const [saving, setSaving] = useState(false)
  const [msg, setMsg] = useState('')

  const fetchData = useCallback(async () => {
    const { data } = await supabase.from('origin_chapters').select('*').order('sort_order')
  ...
  const save = async () => {
    ...
    let error
    if (selected.id) {
      const res = await supabase.from('origin_chapters')
  ...
  const deleteItem = async () => {
    ...
    const { error } = await supabase.from('origin_chapters').delete().eq('id', selected.id)
    if (error) { setMsg(`錯誤: ${error.message}`); return }
    fetchData()
    setSelected(null)
  }

  const moveUp = async (item: OriginChapter) => {
    const idx = list.findIndex(x => x.id === item.id)
    if (idx === 0) return
    const prev = list[idx - 1]
    await supabase.from('origin_chapters').update({ sort_order: item.sort_order }).eq('id', prev.id)
    await supabase.from('origin_chapters').update({ sort_order: prev.sort_order }).eq('id', item.id)
    fetchData()
  }

  const moveDown = async (item: OriginChapter) => {
    const idx = list.findIndex(x => x.id === item.id)
    if (idx === list.length - 1) return
    const next = list[idx + 1]
    await supabase.from('origin_chapters').update({ sort_order: item.sort_order }).eq('id', next.id)
    await supabase.from('origin_chapters').update({ sort_order: next.sort_order }).eq('id', item.id)
    fetchData()
  }

  const newItem = (): OriginChapter => ({
    id: '',
      ...
      <div style={{ padding: 40, maxWidth: 900, margin: 'auto', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2CA', padding: 24, marginBottom: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>章節編號 e.g. 01</label>
            <input
              value={selected.chapter_num}
              onChange={e => setSelected({ ...selected, chapter_num: e.target.value })}
              style={{ width: '100%', padding: '9px 12px', fontSize: 14, border: '1px solid #D8D2CA', borderRadius: 3, outline: 'none', boxSizing: 'border-box' }}
            />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>章節標題 e.g. CHAPTER 01 — ETHIOPIA</label>
            <input
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>標題</label>
            <input
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>細節文字 e.g. Yirgacheffe · 1,900m · Heirloom Varieties</label>
            <input
              value={selected.detail}
              onChange={e => setSelected({ ...selected, detail: e.target.value })}
              style={{ width: '100%', padding: '9px 12px', fontSize: 14, border: '1px solid #D8D2CA', borderRadius: 3, outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
  ...
  return (
    <div style={{ padding: 40, maxWidth: 900, margin: 'auto', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div>
          <p style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#8C7B6B', marginBottom: 4 }}>Terroir CMS</p>
          <h1 style={{ fontSize: 20, fontWeight: 500, color: '#111010', margin: 0 }}>產地故事</h1>
        </div>
        <button
      ...
      <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2CA', marginBottom: 16 }}>
        ...
        {list.map((item, idx) => (
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: idx < list.length - 1 ? '1px solid #EEEBE5' : 'none', backgroundColor: '#fff' }}>
            ...
            <span style={{ fontSize: 13, color: '#8C7B6B', width: 32 }}>{item.chapter_num}</span>
