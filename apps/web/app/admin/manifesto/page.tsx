...
export default function ManifestoAdmin() {
  ...
  useEffect(() => {
    supabase.from('manifesto').select('*').limit(1).single()
  ...
  const save = async () => {
    ...
    setSaving(true)
    setMsg('')
    const { error } = await supabase.from('manifesto')
    ...
    <div style={{ padding: 40, maxWidth: 900, margin: 'auto', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 500, color: '#111010', margin: 0 }}>品牌宣言</h1>
        </div>
      ...
      <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2CA', padding: 24, marginBottom: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>標籤 e.g. MANIFESTO</label>
          <input
            value={data.label}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>引言</label>
          <textarea
            value={data.quote}
            onChange={e => setData({ ...data, quote: e.target.value })}
            style={{ width: '100%', padding: '9px 12px', fontSize: 14, border: '1px solid #D8D2CA', borderRadius: 3, outline: 'none', boxSizing: 'border-box', resize: 'vertical', minHeight: 120 }}
          />
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>署名 e.g. — Terroir Coffee · Taipei · 2019</label>
          <input
