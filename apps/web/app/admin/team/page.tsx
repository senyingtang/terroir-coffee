...
export default function TeamAdmin() {
  ...
  const save = async () => {
    ...
    let error
    if (selected.id) {
      const res = await supabase.from('team')
  ...
  const deleteItem = async () => {
    ...
    const { error } = await supabase.from('team').delete().eq('id', selected.id)
    if (error) { setMsg(`錯誤: ${error.message}`); return }
    fetchData()
    setSelected(null)
  }

  const moveUp = async (item: TeamMember) => {
    const idx = list.findIndex(x => x.id === item.id)
    if (idx === 0) return
    const prev = list[idx - 1]
    await supabase.from('team').update({ sort_order: item.sort_order }).eq('id', prev.id)
    await supabase.from('team').update({ sort_order: prev.sort_order }).eq('id', item.id)
    fetchData()
  }

  const moveDown = async (item: TeamMember) => {
    const idx = list.findIndex(x => x.id === item.id)
    if (idx === list.length - 1) return
    const next = list[idx + 1]
    await supabase.from('team').update({ sort_order: item.sort_order }).eq('id', next.id)
    await supabase.from('team').update({ sort_order: next.sort_order }).eq('id', item.id)
    fetchData()
  }
      ...
      <div style={{ padding: 40, maxWidth: 900, margin: 'auto', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2CA', padding: 24, marginBottom: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>姓名</label>
            <input
              value={selected.name}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>職稱</label>
            <input
              value={selected.role}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: '#5A5147', display: 'block', textTransform: 'uppercase', letterSpacing: '0.05em' }}>個人簡介</label>
            <textarea
              value={selected.bio}
  ...
  return (
    <div style={{ padding: 40, maxWidth: 900, margin: 'auto', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div>
          <p style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#8C7B6B', marginBottom: 4 }}>Terroir CMS</p>
          <h1 style={{ fontSize: 20, fontWeight: 500, color: '#111010', margin: 0 }}>團隊成員</h1>
        </div>
        <button
      ...
      <div style={{ backgroundColor: '#fff', border: '1px solid #D8D2CA', marginBottom: 16 }}>
        ...
        {list.map((item, idx) => (
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: idx < list.length - 1 ? '1px solid #EEEBE5' : 'none', backgroundColor: '#fff' }}>
            ...
            <span style={{ fontSize: 14, color: '#111010', flex: 1 }}>{item.name || '(無姓名)'}</span>
            <span style={{ fontSize: 13, color: '#5A5147' }}>{item.role}</span>
